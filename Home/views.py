...
def handlesignup(request):
    ...
    if request.method == 'POST':
        ...
        print(username,password,conf_password,fname,lname,email_number,shelter,age)

        if password == conf_password:
            try:
                User.objects.create_user(username=username,password=password)
                # password = make_password(password)
                myuser = Customer(Username=username,first_name=fname,last_name=lname,
                    email_number=email_number,shelter=shelter,age=age)
...
def handleRescue(request):
    if request.method == 'POST':
        try:
            ...
            myuser = RescueComplaint.objects.create_user(username=username, email=email, street_address1=adress1, 
